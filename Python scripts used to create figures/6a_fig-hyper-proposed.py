...
no_information_provided = len( [i for i in data["Model opt. procedure"] if i == "No information provided" ]  )
information_provided = len( [i for i in data["Model opt. procedure"] if i != "No information provided" ]  )
...
values = [no_information_provided, 21]
...
path = Path("Python scripts used to create figures/fig-hyper-proposed.pdf")
plt.tight_layout()
plt.savefig(path)
plt.show()
